...
/** Thu / nạp ví: loại tiền cố định, không ràng số dư. */
const WALLET_CREDIT_CURRENCIES: PaymentCurrency[] = [...CASH_FUND_CURRENCY_OPTIONS, "OTHER"];

export type PaymentConfirmDialogCoreProps = {
  open: boolean;
...
export function PaymentConfirmDialogCore(props: PaymentConfirmDialogCoreProps) {
  ...
  const allowedCurrencies: PaymentCurrency[] = React.useMemo(() => {
    ...
    if (source === "CASH") {
      if (walletVariant === "credit") {
        return [...CASH_FUND_CURRENCY_OPTIONS];
      }
      const keys = Object.keys(cashBalances ?? {})
        .map((x) => String(x || "").trim().toUpperCase())
        .filter(Boolean);
      const pos = keys.filter((k) => (Number((cashBalances as Record<string, number>)?.[k] ?? 0) || 0) > 0);
      return pos;
    }
    if (source === "WALLET") {
      if (walletVariant === "credit") {
        return WALLET_CREDIT_CURRENCIES;
      }
  ...
  }, [source, sourceLockedCurrency, cashBalances, wallets, walletKey, walletVariant]);
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (source === "CASH" && allowedCurrencies.length === 0 && walletVariant === "debit") {
                ...
                if (walletVariant === "debit" && source === "WALLET" && walletKey) {
                  const w = wallets.find((x) => x.key === walletKey);
                  const bal = Number((w?.balances as Record<string, number> | undefined)?.[cur] ?? 0) || 0;
                  if (num > bal) {
